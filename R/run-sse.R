...
runSSE <- function(
  ...
  # .computeSSEOutputs() already precomputes and caches the covariance-mode
  # draw-adequacy diagnostic (parameterDrawSummary()) as outputs$parameterDrawSummary
  # -- shared with recomputeSSE(), which needs the same precompute -- rather
  # than leaving every caller to recompute it from x$initialValues on demand.
